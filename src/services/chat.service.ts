...
export class ChatService {
    ...
    async chatAndGenerate(messages: ChatMessage[], userText: string = '', docContent: string = ''): Promise<ChatResponse> {
        let systemContent = `你是一个资深的商业演示文稿（PPT）咨询专家。你的任务是通过对话了解客户的需求，并最终为他们生成一份高质量的PPT内容。
...
请发挥你的专业知识，为用户填充丰满、专业的 bullets 内容。如果还没聊完，不要输出 JSON，继续聊天。`;
        ...
        if (docContent) {
            systemContent = `你是一个资深的商业演示文稿（PPT）咨询专家。用户提供了一份文档，希望你根据这份文档帮他生成PPT。

请分析用户上传的文档内容，然后：
1. 如果文档内容不完整或不足以生成完整的PPT，主动向用户询问补充信息：
   - 这个PPT的用途是？（如：汇报、路演、培训）
   - 受众是谁？（如：管理层、客户、技术人员）
   - 风格偏好？（如：简洁、商务、故事化）
2. 当信息足够时，根据文档内容和用户需求，生成一份专业的PPT大纲和内容。
3. **输出格式**：在你回复的话术最后，**必须**输出一段 JSON 格式的数据，用 \`\`\`json 和 \`\`\` 包裹起来。
...
\`\`\`
注意：slideRole 可以是 "agenda"(目录), "content"(正文), "comparison"(对比), "summary"(总结), "next_step"(下一步计划)。
请基于文档内容生成专业、逻辑清晰的PPT大纲。如果还没聊完，不要输出 JSON，继续聊天。`;
        }
